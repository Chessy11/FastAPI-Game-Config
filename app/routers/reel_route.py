...
from app.cruds import reel_crud
...
async def create_reel_config(
    ...
    try:
        ...
        new_reel = await reel_crud.create_reel(session, create_reel, current_user.user_id)
...
async def get_reels_by_game_id(
    ...
    reels = await reel_crud.get_reels_by_game_id(session, game_id, current_user.user_id)
    return reels
...
async def get_reel_by_id(
    ...
    reel = await reel_crud.get_reel_by_id(reel_id, current_user.user_id, session)
    if reel is None:
...
async def add_symbol_to_reel(add_symbol: ReelSymbolInSchema,
    ...
    try:
        new_symbol = await reel_crud.add_symbols_to_reel(session, add_symbol, current_user.user_id)
    except IntegrityError as ie:
        raise HTTPException(status_code=400, detail=str(ie.orig))
    return new_symbol
