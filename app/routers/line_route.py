...
from app.cruds import lines_crud
...
async def create_line_config(create_line: LineInSchema, 
    ...
    try:
        new_line = await lines_crud.create_line(session, create_line, current_user.user_id)
...
async def get_lines_by_game_id(game_id: int, 
                               current_user: UserOutSchema = Depends(get_current_active_user),
                               session: AsyncSession = Depends(get_session)):
    lines = await lines_crud.get_lines_by_game_id(session, game_id, current_user.user_id)
    return lines
...
async def get_line_by_id(line_id: int,
                         current_user: UserOutSchema = Depends(get_current_active_user), 
                         session: AsyncSession = Depends(get_session)):
    line = await lines_crud.get_line_by_id(line_id, session, current_user.user_id)
    if line is None:
...
async def delete_line_by_id(line_id: int, 
                            ...
                            session: AsyncSession = Depends(get_session)):
    try:
        deleted_count = await lines_crud.delete_line_by_id(session, line_id, current_user.user_id)
        if deleted_count == 0:
            raise HTTPException(status_code=404, detail="Line not found")
